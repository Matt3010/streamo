# Graph Report - Streamo  (2026-07-06)

## Corpus Check
- 177 files · ~132,561 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 2117 nodes · 4149 edges · 175 communities (109 shown, 66 thin omitted)
- Extraction: 89% EXTRACTED · 11% INFERRED · 0% AMBIGUOUS · INFERRED: 457 edges (avg confidence: 0.8)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `2698bf27`
- Run `git rev-parse HEAD` and compare to check if the graph is stale.
- Run `graphify update .` after code changes (no API cost).

## Community Hubs (Navigation)
- [[_COMMUNITY_TMDB DTO Models|TMDB DTO Models]]
- [[_COMMUNITY_Provider Client Resolution|Provider Client Resolution]]
- [[_COMMUNITY_Brand Buttons & TMDB Reviews|Brand Buttons & TMDB Reviews]]
- [[_COMMUNITY_Settings DataStore|Settings DataStore]]
- [[_COMMUNITY_Download Quality Selection|Download Quality Selection]]
- [[_COMMUNITY_Backup Manager & JSON ImportExport|Backup Manager & JSON Import/Export]]
- [[_COMMUNITY_DLNA Cast Manager|DLNA Cast Manager]]
- [[_COMMUNITY_Search Screen & Glass Filters|Search Screen & Glass Filters]]
- [[_COMMUNITY_LAN Cast Client (app-to-app)|LAN Cast Client (app-to-app)]]
- [[_COMMUNITY_AnimeUnity Models|AnimeUnity Models]]
- [[_COMMUNITY_Settings ViewModel|Settings ViewModel]]
- [[_COMMUNITY_Chromecast Manager|Chromecast Manager]]
- [[_COMMUNITY_AppRepository (Room facade)|AppRepository (Room facade)]]
- [[_COMMUNITY_Glass Card Composables|Glass Card Composables]]
- [[_COMMUNITY_LAN Cast Receiver (TV side)|LAN Cast Receiver (TV side)]]
- [[_COMMUNITY_Download DAO|Download DAO]]
- [[_COMMUNITY_Player ViewModel & MediaSession|Player ViewModel & MediaSession]]
- [[_COMMUNITY_Series Downloads ViewModel|Series Downloads ViewModel]]
- [[_COMMUNITY_Glass Dialogs & Alerts|Glass Dialogs & Alerts]]
- [[_COMMUNITY_Cast Controller (session locks)|Cast Controller (session locks)]]
- [[_COMMUNITY_Image Placeholder & TV Media Card|Image Placeholder & TV Media Card]]
- [[_COMMUNITY_Download Entry & Downloads VM|Download Entry & Downloads VM]]
- [[_COMMUNITY_TV Player Screen|TV Player Screen]]
- [[_COMMUNITY_History DAO|History DAO]]
- [[_COMMUNITY_Cast Banner UI|Cast Banner UI]]
- [[_COMMUNITY_History ViewModel & Filters|History ViewModel & Filters]]
- [[_COMMUNITY_Cache Management ViewModel|Cache Management ViewModel]]
- [[_COMMUNITY_Watchlist DAO|Watchlist DAO]]
- [[_COMMUNITY_TV Search Screen & Focus|TV Search Screen & Focus]]
- [[_COMMUNITY_NavRoutes (type-safe navigation)|NavRoutes (type-safe navigation)]]
- [[_COMMUNITY_Player ViewModel Cast Modes|Player ViewModel Cast Modes]]
- [[_COMMUNITY_Cast Banner & Section List VMs|Cast Banner & Section List VMs]]
- [[_COMMUNITY_Anime Detail Screen|Anime Detail Screen]]
- [[_COMMUNITY_Vixcloud HLS Client|Vixcloud HLS Client]]
- [[_COMMUNITY_Glass Top Bar|Glass Top Bar]]
- [[_COMMUNITY_TV Navigation Drawer|TV Navigation Drawer]]
- [[_COMMUNITY_Progress DAO|Progress DAO]]
- [[_COMMUNITY_TMDB Cache DAO|TMDB Cache DAO]]
- [[_COMMUNITY_Watchlist ViewModel|Watchlist ViewModel]]
- [[_COMMUNITY_AppDatabase & DI Module|AppDatabase & DI Module]]
- [[_COMMUNITY_Cluster 40|Cluster 40]]
- [[_COMMUNITY_Cluster 41|Cluster 41]]
- [[_COMMUNITY_Cluster 42|Cluster 42]]
- [[_COMMUNITY_Cluster 43|Cluster 43]]
- [[_COMMUNITY_Cluster 44|Cluster 44]]
- [[_COMMUNITY_Cluster 45|Cluster 45]]
- [[_COMMUNITY_Cluster 46|Cluster 46]]
- [[_COMMUNITY_Cluster 47|Cluster 47]]
- [[_COMMUNITY_Cluster 48|Cluster 48]]
- [[_COMMUNITY_Cluster 49|Cluster 49]]
- [[_COMMUNITY_Cluster 50|Cluster 50]]
- [[_COMMUNITY_Cluster 51|Cluster 51]]
- [[_COMMUNITY_Cluster 52|Cluster 52]]
- [[_COMMUNITY_Cluster 53|Cluster 53]]
- [[_COMMUNITY_Cluster 54|Cluster 54]]
- [[_COMMUNITY_Cluster 55|Cluster 55]]
- [[_COMMUNITY_Cluster 56|Cluster 56]]
- [[_COMMUNITY_Cluster 57|Cluster 57]]
- [[_COMMUNITY_Cluster 58|Cluster 58]]
- [[_COMMUNITY_Cluster 59|Cluster 59]]
- [[_COMMUNITY_Cluster 60|Cluster 60]]
- [[_COMMUNITY_Cluster 61|Cluster 61]]
- [[_COMMUNITY_Cluster 62|Cluster 62]]
- [[_COMMUNITY_Cluster 63|Cluster 63]]
- [[_COMMUNITY_Cluster 64|Cluster 64]]
- [[_COMMUNITY_Cluster 65|Cluster 65]]
- [[_COMMUNITY_Cluster 66|Cluster 66]]
- [[_COMMUNITY_Cluster 67|Cluster 67]]
- [[_COMMUNITY_Cluster 68|Cluster 68]]
- [[_COMMUNITY_Cluster 69|Cluster 69]]
- [[_COMMUNITY_Cluster 70|Cluster 70]]
- [[_COMMUNITY_Cluster 71|Cluster 71]]
- [[_COMMUNITY_Cluster 72|Cluster 72]]
- [[_COMMUNITY_Cluster 73|Cluster 73]]
- [[_COMMUNITY_Cluster 74|Cluster 74]]
- [[_COMMUNITY_Cluster 75|Cluster 75]]
- [[_COMMUNITY_Cluster 76|Cluster 76]]
- [[_COMMUNITY_Cluster 77|Cluster 77]]
- [[_COMMUNITY_Cluster 78|Cluster 78]]
- [[_COMMUNITY_Cluster 79|Cluster 79]]
- [[_COMMUNITY_Cluster 80|Cluster 80]]
- [[_COMMUNITY_Cluster 81|Cluster 81]]
- [[_COMMUNITY_Cluster 82|Cluster 82]]
- [[_COMMUNITY_Cluster 83|Cluster 83]]
- [[_COMMUNITY_Cluster 84|Cluster 84]]
- [[_COMMUNITY_Cluster 85|Cluster 85]]
- [[_COMMUNITY_Cluster 86|Cluster 86]]
- [[_COMMUNITY_Cluster 87|Cluster 87]]
- [[_COMMUNITY_Cluster 88|Cluster 88]]
- [[_COMMUNITY_.loadCurrentSource|.loadCurrentSource]]
- [[_COMMUNITY_Cluster 90|Cluster 90]]
- [[_COMMUNITY_Cluster 91|Cluster 91]]
- [[_COMMUNITY_Cluster 92|Cluster 92]]
- [[_COMMUNITY_Cluster 93|Cluster 93]]
- [[_COMMUNITY_Cluster 94|Cluster 94]]
- [[_COMMUNITY_Cluster 95|Cluster 95]]
- [[_COMMUNITY_SectionHeader|SectionHeader]]
- [[_COMMUNITY_Size|Size]]
- [[_COMMUNITY_Cluster 98|Cluster 98]]
- [[_COMMUNITY_Cluster 99|Cluster 99]]
- [[_COMMUNITY_Cluster 100|Cluster 100]]
- [[_COMMUNITY_Cluster 101|Cluster 101]]
- [[_COMMUNITY_Cluster 102|Cluster 102]]
- [[_COMMUNITY_TvAnimeDetailScreen|TvAnimeDetailScreen]]
- [[_COMMUNITY_Cluster 104|Cluster 104]]
- [[_COMMUNITY_Cluster 105|Cluster 105]]
- [[_COMMUNITY_Cluster 106|Cluster 106]]
- [[_COMMUNITY_Cluster 107|Cluster 107]]
- [[_COMMUNITY_Cluster 108|Cluster 108]]
- [[_COMMUNITY_Cluster 109|Cluster 109]]
- [[_COMMUNITY_Cluster 110|Cluster 110]]
- [[_COMMUNITY_Cluster 111|Cluster 111]]
- [[_COMMUNITY_Cluster 112|Cluster 112]]
- [[_COMMUNITY_Cluster 113|Cluster 113]]
- [[_COMMUNITY_Cluster 114|Cluster 114]]
- [[_COMMUNITY_Cluster 115|Cluster 115]]
- [[_COMMUNITY_Cluster 116|Cluster 116]]
- [[_COMMUNITY_Cluster 117|Cluster 117]]
- [[_COMMUNITY_Cluster 118|Cluster 118]]
- [[_COMMUNITY_Cluster 124|Cluster 124]]
- [[_COMMUNITY_Cluster 125|Cluster 125]]
- [[_COMMUNITY_Cluster 126|Cluster 126]]
- [[_COMMUNITY_Cluster 128|Cluster 128]]
- [[_COMMUNITY_Cluster 130|Cluster 130]]
- [[_COMMUNITY_Piano Modali glass con blur reale (inline overlay)|Piano: Modali glass con blur reale (inline overlay)]]
- [[_COMMUNITY_Piano Allineare Salta intro all'implementazione iOS|Piano: Allineare "Salta intro" all'implementazione iOS]]
- [[_COMMUNITY_PlayerViewModel.kt|PlayerViewModel.kt]]
- [[_COMMUNITY_SeasonChip|SeasonChip]]
- [[_COMMUNITY_Repository Guidelines|Repository Guidelines]]
- [[_COMMUNITY_Streamo single-module app Gradle project structure|Streamo single-module :app Gradle project structure]]
- [[_COMMUNITY_warpkit.aar (Cloudflare WARP tunnel via wireproxykit)|warpkit.aar (Cloudflare WARP tunnel via wireproxykit)]]
- [[_COMMUNITY_WarpEngine reflects Wireproxykit class at runtime (graceful absence)|WarpEngine reflects Wireproxykit class at runtime (graceful absence)]]
- [[_COMMUNITY_HistoryEntry composite primary key schema (tmdbId, mediaType, season, episode, watchedDay)|HistoryEntry composite primary key schema (tmdbId, mediaType, season, episode, watchedDay)]]
- [[_COMMUNITY_iOS history model (per-day row, snapshot, de-dupe by coordinate)|iOS history model (per-day row, snapshot, de-dupe by coordinate)]]
- [[_COMMUNITY_totalWatchSeconds de-duplication by coordinate (iOS parity)|totalWatchSeconds de-duplication by coordinate (iOS parity)]]
- [[_COMMUNITY_ContinueWatchingScreen + ViewModel (full list)|ContinueWatchingScreen + ViewModel (full list)]]
- [[_COMMUNITY_NavRoutes.SectionList(title, endpoint, mediaType)|NavRoutes.SectionList(title, endpoint, mediaType)]]
- [[_COMMUNITY_TMDBApi.list() page parameter pagination|TMDBApi.list() page parameter pagination]]
- [[_COMMUNITY_Inline Haze overlay GlassDialog (no Dialog window, shares hazeSource)|Inline Haze overlay GlassDialog (no Dialog window, shares hazeSource)]]
- [[_COMMUNITY_Haze nested-source SIGSEGV constraint|Haze nested-source SIGSEGV constraint]]
- [[_COMMUNITY_SKIP_INTRO_SECONDS DataStore preference|SKIP_INTRO_SECONDS DataStore preference]]
- [[_COMMUNITY_IntroSkipClient (api.theintrodb.orgv3media)|IntroSkipClient (api.theintrodb.org/v3/media)]]
- [[_COMMUNITY_iOS PlaybackController.swift skip-segment reference|iOS PlaybackController.swift skip-segment reference]]
- [[_COMMUNITY_SkipPrompt introcredits + auto-advance countdown (iOS parity)|SkipPrompt intro/credits + auto-advance countdown (iOS parity)]]
- [[_COMMUNITY_NavRoutes.Library (collapsed Watchlist+History+ContinueWatching)|NavRoutes.Library (collapsed Watchlist+History+ContinueWatching)]]
- [[_COMMUNITY_Reuse all ViewModelsRoomDIprovider unchanged on TV|Reuse all ViewModels/Room/DI/provider unchanged on TV]]
- [[_COMMUNITY_Single APK dual launcher (LAUNCHER + LEANBACK_LAUNCHER) with runtime form-factor detection|Single APK dual launcher (LAUNCHER + LEANBACK_LAUNCHER) with runtime form-factor detection]]
- [[_COMMUNITY_TvFocusModifiers (scale + white border on focus)|TvFocusModifiers (scale + white border on focus)]]
- [[_COMMUNITY_TvPlayerScreen D-pad key handling overlay (useController=false)|TvPlayerScreen D-pad key handling overlay (useController=false)]]
- [[_COMMUNITY_TvRootView (ModalNavigationDrawer nav rail HomeCercaLibreriaImpostazioni)|TvRootView (ModalNavigationDrawer nav rail: Home/Cerca/Libreria/Impostazioni)]]
- [[_COMMUNITY_BrandRed hardcoded breaks user accent color selection|BrandRed hardcoded breaks user accent color selection]]
- [[_COMMUNITY_GlassDefaults.BlurRadiusChromeBlurRadiusDialog tokens|GlassDefaults.BlurRadiusChrome/BlurRadiusDialog tokens]]
- [[_COMMUNITY_PressFeedback fragmentation (MediaCardProgressMediaCard vs SectionHeaderBrandButtons reimplementation)|PressFeedback fragmentation (MediaCard/ProgressMediaCard vs SectionHeader/BrandButtons reimplementation)]]
- [[_COMMUNITY_uithemeShape.kt token scale (xssmmdlgxlpill)|ui/theme/Shape.kt token scale (xs/sm/md/lg/xl/pill)]]
- [[_COMMUNITY_AdvancedSettingsScreen split + provider locale UI|AdvancedSettingsScreen split + provider locale UI]]
- [[_COMMUNITY_MediaCard info (yearrating) + SHOW_CARD_INFO toggle|MediaCard info (year/rating) + SHOW_CARD_INFO toggle]]
- [[_COMMUNITY_Home Hero carousel (auto-advance, swipe, indicators, play+bookmark)|Home Hero carousel (auto-advance, swipe, indicators, play+bookmark)]]
- [[_COMMUNITY_iOS LAN sharing (NanoHTTPDQRVLCpassword) excluded from porting scope|iOS LAN sharing (NanoHTTPD/QR/VLC/password) excluded from porting scope]]
- [[_COMMUNITY_STREAMING_QUALITY pref + ExoPlayer setMaxVideoSize|STREAMING_QUALITY pref + ExoPlayer setMaxVideoSize]]
- [[_COMMUNITY_Top 10 row with shaped numerals (outline Stroke)|Top 10 row with shaped numerals (outline Stroke)]]
- [[_COMMUNITY_WARP badge in player loading overlay (port of iOS WarpBadge.swift)|WARP badge in player loading overlay (port of iOS WarpBadge.swift)]]
- [[_COMMUNITY_Watchlist filters (type + status derived from progress)|Watchlist filters (type + status derived from progress)]]
- [[_COMMUNITY_Cast icon + persistent notification for player management|Cast icon + persistent notification for player management]]
- [[_COMMUNITY_Continue Watching dedupe issue (last episode of last season watched)|Continue Watching dedupe issue (last episode of last season watched)]]
- [[_COMMUNITY_Download stopplay control fix|Download stop/play control fix]]
- [[_COMMUNITY_Skeleton flicker on switching season in series detail|Skeleton flicker on switching season in series detail]]
- [[_COMMUNITY_HeroControls|HeroControls]]

## God Nodes (most connected - your core abstractions)
1. `Text` - 122 edges
2. `PlayerViewModel` - 86 edges
3. `TmdbItem` - 55 edges
4. `DownloadEntry` - 47 edges
5. `AppRepository` - 46 edges
6. `SettingsViewModel` - 44 edges
7. `DetailViewModel` - 33 edges
8. `SeriesDownloadsViewModel` - 33 edges
9. `ChromecastManager` - 32 edges
10. `TvFocusable()` - 31 edges

## Surprising Connections (you probably didn't know these)
- `TvRootView()` --calls--> `NavigationDrawer()`  [INFERRED]
  app/src/main/java/com/streamo/app/ui/tv/TvRootView.kt → androidx/tv/material3/NavigationDrawer.kt
- `TvRootView()` --calls--> `rememberDrawerState()`  [INFERRED]
  app/src/main/java/com/streamo/app/ui/tv/TvRootView.kt → androidx/tv/material3/NavigationDrawer.kt
- `AppNavHost()` --calls--> `AnimeDetailScreen()`  [INFERRED]
  app/src/main/java/com/streamo/app/navigation/AppNavHost.kt → app/src/main/java/com/streamo/app/ui/anime/AnimeDetailScreen.kt
- `AppNavHost()` --calls--> `AnimeScreen()`  [INFERRED]
  app/src/main/java/com/streamo/app/navigation/AppNavHost.kt → app/src/main/java/com/streamo/app/ui/anime/AnimeScreen.kt
- `AppNavHost()` --calls--> `ContinueWatchingScreen()`  [INFERRED]
  app/src/main/java/com/streamo/app/navigation/AppNavHost.kt → app/src/main/java/com/streamo/app/ui/continuewatching/ContinueWatchingScreen.kt

## Import Cycles
- None detected.

## Hyperedges (group relationships)
- **TV plan reused-architecture group (single APK + form-factor branch + ViewModel reuse)** — plans_android_tv_support_plan_single_apk_dual_launcher, plans_android_tv_support_plan_formfactor_istvdevice, plans_android_tv_support_plan_reuse_viewmodels_unchanged, plans_android_tv_support_plan_tvrootview [EXTRACTED 0.95]
- **Style token consolidation (Shape + Alpha + BlurRadius tokens)** — plans_animation_style_audit_plan_shape_tokens, plans_animation_style_audit_plan_alpha_tokens, plans_animation_style_audit_plan_blur_radius_tokens [INFERRED 0.85]
- **Skip Intro MVP -> iOS-aligned progression** — claude_plans_skip_intro_button_fixed_duration_mvp, claude_plans_skip_intro_button_skip_intro_seconds_pref, claude_plans_skip_intro_ios_alignment_introskipclient, claude_plans_skip_intro_ios_alignment_skip_prompt_credits [EXTRACTED 0.95]

## Communities (175 total, 66 thin omitted)

### Community 0 - "TMDB DTO Models"
Cohesion: 0.06
Nodes (39): Int, String, stub(), TmdbCastMember, TmdbCredits, TmdbEpisodeDetail, TmdbEpisodeRef, TmdbGenreListResponse (+31 more)

### Community 1 - "Provider Client Resolution"
Cohesion: 0.06
Nodes (52): decodeHTMLEntities(), extractYear(), firstMatch(), HttpResponse, Class, Int, List, Long (+44 more)

### Community 2 - "Brand Buttons & TMDB Reviews"
Cohesion: 0.21
Nodes (22): aspectLabel(), androidx, Boolean, com, Float, FocusRequester, ImageVector, Int (+14 more)

### Community 3 - "Settings DataStore"
Cohesion: 0.05
Nodes (21): Boolean, Context, Float, Flow, Map, String, Triple, SettingsDataStore (+13 more)

### Community 4 - "Download Quality Selection"
Cohesion: 0.06
Nodes (22): Ask, Cap, capHeightFromEntryQuality(), DownloadQualityPref, Int, String, Max, NetworkType (+14 more)

### Community 5 - "Backup Manager & JSON Import/Export"
Cohesion: 0.07
Nodes (28): BackupManager, BackupPayload, Unit, Uri, cancel(), enqueue(), Boolean, Context (+20 more)

### Community 6 - "DLNA Cast Manager"
Cohesion: 0.11
Nodes (18): DlnaCastManager, DlnaPosition, DlnaRenderer, Boolean, Context, InetAddress, List, Long (+10 more)

### Community 7 - "Search Screen & Glass Filters"
Cohesion: 0.10
Nodes (13): Flow, List, String, SearchHistoryDao, SearchHistoryEntry, Boolean, Collection, Int (+5 more)

### Community 8 - "LAN Cast Client (app-to-app)"
Cohesion: 0.08
Nodes (20): Boolean, Context, List, Long, String, LanCastClient, Context, Int (+12 more)

### Community 9 - "AnimeUnity Models"
Cohesion: 0.10
Nodes (22): AUAnime, AUEpisode, AUEpisodePage, AUInfoResponse, AURecordsResponse, Boolean, Int, String (+14 more)

### Community 10 - "Settings ViewModel"
Cohesion: 0.07
Nodes (9): Boolean, Float, Int, Long, StateFlow, String, Triple, Uri (+1 more)

### Community 11 - "Chromecast Manager"
Cohesion: 0.09
Nodes (17): ChromecastManager, Boolean, InetAddress, Int, List, Long, Map, OkHttpClient (+9 more)

### Community 12 - "AppRepository (Room facade)"
Cohesion: 0.06
Nodes (17): HistoryDao, Flow, Int, List, Long, String, HistoryEntry, Long (+9 more)

### Community 13 - "Glass Card Composables"
Cohesion: 0.19
Nodes (22): downloadDetailLine(), downloadItemLabel(), DownloadManagerRow(), DownloadsSheet(), formatBytes(), formatSpeed(), Boolean, Float (+14 more)

### Community 14 - "LAN Cast Receiver (TV side)"
Cohesion: 0.10
Nodes (19): Boolean, Int, StateFlow, String, LanCastReceiver, Any, IHTTPSession, Response (+11 more)

### Community 15 - "Download DAO"
Cohesion: 0.24
Nodes (15): buildCastDeviceGroups(), extractIp(), androidx, Boolean, Int, List, Modifier, String (+7 more)

### Community 16 - "Player ViewModel & MediaSession"
Cohesion: 0.09
Nodes (10): PlaybackSource, Context, ExoPlayer, Float, Job, List, MediaSession, StateFlow (+2 more)

### Community 17 - "Series Downloads ViewModel"
Cohesion: 0.05
Nodes (26): DownloadDao, Boolean, Float, Flow, Int, List, Long, String (+18 more)

### Community 18 - "Glass Dialogs & Alerts"
Cohesion: 0.19
Nodes (21): DownloadQualityRequest, cardEnter(), cardExit(), DialogHandle, GlassAlertDialog(), GlassDialog(), GlassDialogContent(), GlassDialogDestructiveButton() (+13 more)

### Community 19 - "Cast Controller (session locks)"
Cohesion: 0.18
Nodes (8): android, CastController, Boolean, Job, List, Long, MediaSession, StateFlow

### Community 20 - "Image Placeholder & TV Media Card"
Cohesion: 0.07
Nodes (29): ImagePlaceholder(), androidx, Boolean, Modifier, String, com, List, TvAnimeCatalogCard() (+21 more)

### Community 21 - "Download Entry & Downloads VM"
Cohesion: 0.14
Nodes (13): Boolean, Double, Dp, Float, Int, Modifier, String, Unit (+5 more)

### Community 22 - "TV Player Screen"
Cohesion: 0.26
Nodes (14): Text, CastDeviceGroup, Boolean, List, String, CastDetailPanel(), CastDeviceRow(), CastPickerDialog() (+6 more)

### Community 23 - "History DAO"
Cohesion: 0.18
Nodes (22): TmdbReview, DetailContent(), DetailScreen(), DetailScrollContent(), DetailSkeleton(), EpisodeCard(), EpisodesSection(), EpisodesUnavailable() (+14 more)

### Community 24 - "Cast Banner UI"
Cohesion: 0.17
Nodes (25): TmdbGenre, FilterBar(), FilterButton(), GenreBadgeRow(), GenrePickerDialog(), GenreSummaryChip(), Int, List (+17 more)

### Community 25 - "History ViewModel & Filters"
Cohesion: 0.18
Nodes (14): coordinate(), formatWatchTime(), HistoryFilter, HistoryItem, HistorySection, HistoryUiState, HistoryViewModel, Double (+6 more)

### Community 26 - "Cache Management ViewModel"
Cohesion: 0.16
Nodes (7): CacheManagementViewModel, Boolean, Collection, Int, List, Long, StateFlow

### Community 27 - "Watchlist DAO"
Cohesion: 0.15
Nodes (7): Boolean, Flow, Int, List, String, WatchlistDao, WatchlistEntry

### Community 28 - "TV Search Screen & Focus"
Cohesion: 0.07
Nodes (28): 0.1 `gradle/libs.versions.toml`, 0.2 `app/build.gradle.kts`, 0.3 `app/src/main/AndroidManifest.xml`, 0.4 Banner asset, 1.1 NEW `app/src/main/java/com/streamo/app/util/FormFactor.kt`, 1.2 Edit `MainActivity.kt`, 2.1 Edit `navigation/NavRoutes.kt` — add ONE route, 2.2 NEW `ui/tv/TvAppNavHost.kt` (+20 more)

### Community 29 - "NavRoutes (type-safe navigation)"
Cohesion: 0.12
Nodes (29): AppNavHost(), detailEnterTransition(), detailExitTransition(), isDetailRoute(), Boolean, EnterTransition, ExitTransition, Int (+21 more)

### Community 30 - "Player ViewModel Cast Modes"
Cohesion: 0.24
Nodes (3): ChromecastRenderer, Boolean, onIsPlayingChanged()

### Community 31 - "Cast Banner & Section List VMs"
Cohesion: 0.12
Nodes (10): CastBannerViewModel, ContinueWatchingViewModel, Int, List, StateFlow, Int, Long, String (+2 more)

### Community 32 - "Anime Detail Screen"
Cohesion: 0.31
Nodes (17): FocusRequester, TvFocusable(), Boolean, Int, List, Modifier, String, TvFilterBar() (+9 more)

### Community 33 - "Vixcloud HLS Client"
Cohesion: 0.24
Nodes (9): FetchFailed, List, OkHttpClient, String, PlaylistNotFound, StreamEntry, VixcloudClient, VixError (+1 more)

### Community 34 - "Glass Top Bar"
Cohesion: 0.17
Nodes (17): glassCapsule(), GlassLargeTitle(), GlassTopBar(), GlassTopBarScaffold(), androidx, HazeState, ImageVector, Modifier (+9 more)

### Community 35 - "TV Navigation Drawer"
Cohesion: 0.21
Nodes (14): DrawerSheet(), DrawerState, DrawerValue, Modifier, Unit, ModalNavigationDrawer(), NavigationDrawer(), rememberDrawerState() (+6 more)

### Community 36 - "Progress DAO"
Cohesion: 0.21
Nodes (6): Flow, Int, List, String, ProgressDao, ProgressEntry

### Community 37 - "TMDB Cache DAO"
Cohesion: 0.07
Nodes (14): AppDatabase, migrate(), Int, ProviderMappingDao, Int, Long, String, TmdbCacheDao (+6 more)

### Community 38 - "Watchlist ViewModel"
Cohesion: 0.20
Nodes (9): Int, List, StateFlow, String, WatchlistItem, WatchlistStatusFilter, WatchlistType, WatchlistViewModel (+1 more)

### Community 39 - "AppDatabase & DI Module"
Cohesion: 0.18
Nodes (7): DlnaSessionPlayer, Boolean, Int, Long, ListenableFuture, SimpleBasePlayer, State

### Community 40 - "Cluster 40"
Cohesion: 0.14
Nodes (10): ensureNotificationChannel(), androidx, Context, DownloadManager, Int, Notification, MediaDownloadService, Download (+2 more)

### Community 41 - "Cluster 41"
Cohesion: 0.29
Nodes (4): Boolean, Int, String, TmdbCacheKey

### Community 42 - "Cluster 42"
Cohesion: 0.18
Nodes (7): HomeViewModel, Boolean, com, Int, List, StateFlow, String

### Community 43 - "Cluster 43"
Cohesion: 0.28
Nodes (14): AllEpisodesContent(), DownloadedOnlyContent(), EpisodeDownloadCard(), androidx, Boolean, ImageVector, Int, List (+6 more)

### Community 44 - "Cluster 44"
Cohesion: 0.24
Nodes (7): Any, Boolean, Class, String, Throwable, WarpEngine, Method

### Community 45 - "Cluster 45"
Cohesion: 0.27
Nodes (9): GlassCard(), GlassDefaults, GlassFilterChip(), GlassSnapshot, androidx, Boolean, Modifier, Shape (+1 more)

### Community 46 - "Cluster 46"
Cohesion: 0.24
Nodes (14): BrandButton(), BrandButtonDefaults, BrandIconButton(), BrandSecondaryButton(), Boolean, ImageVector, Modifier, String (+6 more)

### Community 47 - "Cluster 47"
Cohesion: 0.29
Nodes (7): Boolean, Dp, FocusRequester, List, Modifier, TvLoadMoreCard(), TvSectionRow()

### Community 48 - "Cluster 48"
Cohesion: 0.15
Nodes (17): CastBanner(), Boolean, HazeState, Modifier, String, RootTabView(), androidx, Boolean (+9 more)

### Community 49 - "Cluster 49"
Cohesion: 0.21
Nodes (8): AnimeDetailViewModel, Boolean, Int, List, Map, StateFlow, String, IntRange

### Community 50 - "Cluster 50"
Cohesion: 0.21
Nodes (10): Boolean, Double, Dp, Float, Int, Modifier, String, MediaCard() (+2 more)

### Community 51 - "Cluster 51"
Cohesion: 0.24
Nodes (7): androidx, Int, Pair, onPlaybackStateChanged(), onTracksChanged(), onVideoSizeChanged(), TrackInfo

### Community 52 - "Cluster 52"
Cohesion: 0.24
Nodes (11): Anime, GlassBottomBar(), Home, Boolean, Color, HazeState, Modifier, rememberTabContentColor() (+3 more)

### Community 53 - "Cluster 53"
Cohesion: 0.21
Nodes (6): AnimeResumeTarget, AnimeViewModel, Job, List, StateFlow, String

### Community 54 - "Cluster 54"
Cohesion: 0.11
Nodes (18): 1. `HistoryEntry` ha chiave primaria sbagliata (`tmdbId` solo), 2. Mancano gli snapshot giornalieri di progresso/durata, 3. La rimozione cancella l'intera serie, 4. Il totale "Tempo guardato" non de-duplica per coordinata, Bug trovati, Considerazioni / rischi, DAO e Repository, File coinvolti (+10 more)

### Community 55 - "Cluster 55"
Cohesion: 0.48
Nodes (6): formatClock(), Double, String, TvDetailScreen(), TvEpisodeCard(), TvProviderPicker()

### Community 57 - "Cluster 57"
Cohesion: 0.29
Nodes (10): AnimatedActionIcon(), ContinueWatchingRow(), ErrorState(), HomeScreen(), Boolean, ImageVector, Int, List (+2 more)

### Community 58 - "Cluster 58"
Cohesion: 0.35
Nodes (10): hsvColor(), Color, Float, String, Triple, QualityPickerRow(), rgbToHsv(), SectionHeader() (+2 more)

### Community 59 - "Cluster 59"
Cohesion: 0.42
Nodes (8): CastMedia, CastSession, Chromecast, Dlna, Map, String, Lan, okhttp3

### Community 60 - "Cluster 60"
Cohesion: 0.20
Nodes (6): DownloadStateSyncer, DownloadManager, ImageLoader, MainApplication, Application, ImageLoaderFactory

### Community 61 - "Cluster 61"
Cohesion: 0.36
Nodes (8): AnimeCatalogCard(), AnimeContinueRow(), AnimeErrorState(), AnimeScreen(), AnimeSearchField(), List, Modifier, String

### Community 62 - "Cluster 62"
Cohesion: 0.36
Nodes (6): DownloadResolutionProbe, DataSource, Int, List, OkHttpClient, String

### Community 63 - "Cluster 63"
Cohesion: 0.24
Nodes (5): Int, Intent, MediaSession, PlaybackService, MediaSessionService

### Community 64 - "Cluster 64"
Cohesion: 0.50
Nodes (4): Boolean, String, TvButton(), TvLogViewerScreen()

### Community 65 - "Cluster 65"
Cohesion: 0.11
Nodes (18): 1.1 Gating incoerente su "riduci effetti" (`LocalReducedEffects`), 1.2 Frammentazione del "press feedback", 1.3 Famiglie di durata/easing incoerenti nella navigazione, 1.4 Schermate senza alcuna animazione locale, 1.5 Nota strutturale (non bloccante per la fase 1, ma impatta il codice di animazione), 2.1 Assenza di un file di token in `ui/theme/`, 2.2 Sprawl della scala di alpha, 2.3 Raggio di blur (Haze) incoerente (+10 more)

### Community 66 - "Cluster 66"
Cohesion: 0.29
Nodes (4): Int, Long, StateFlow, TvCacheManagementViewModel

### Community 67 - "Cluster 67"
Cohesion: 0.28
Nodes (6): AppTheme(), Color, isTabletDevice(), isTvDevice(), Boolean, Bundle

### Community 68 - "Cluster 68"
Cohesion: 0.33
Nodes (8): HeroControls(), HeroPage(), HomeHero(), Boolean, Float, Int, List, Modifier

### Community 69 - "Cluster 69"
Cohesion: 0.25
Nodes (6): CastOptionsProvider, Context, List, CastOptions, OptionsProvider, SessionProvider

### Community 70 - "Cluster 70"
Cohesion: 0.28
Nodes (7): IntroSkipClient, Boolean, Int, Long, MediaResponse, Segment, Segments

### Community 71 - "Cluster 71"
Cohesion: 0.12
Nodes (15): Architecture, Build & test commands, Cards & posters, Confirmation dialogs, Downloads (two paths — be careful which you touch), graphify, Language, Playback & casting (`player/`) (+7 more)

### Community 72 - "Cluster 72"
Cohesion: 0.42
Nodes (8): EmptyMessage(), HistoryScreen(), Boolean, Modifier, String, SectionTitle(), TypeChip(), WatchTimeCard()

### Community 74 - "Cluster 74"
Cohesion: 0.33
Nodes (8): Boolean, ImageVector, Modifier, String, Unit, TvImmersiveRow(), TvSectionTitle(), LazyListState

### Community 75 - "Cluster 75"
Cohesion: 0.32
Nodes (6): DownloadInfrastructure, Context, DataSource, DownloadManager, CacheDataSource, SimpleCache

### Community 76 - "Cluster 76"
Cohesion: 0.32
Nodes (3): String, Throwable, ProviderDebugLogger

### Community 77 - "Cluster 77"
Cohesion: 0.31
Nodes (9): GlassBottomSheet(), GlassBottomSheetContent(), Boolean, EnterTransition, ExitTransition, HazeState, SheetAnchor, sheetCardEnter() (+1 more)

### Community 78 - "Cluster 78"
Cohesion: 0.67
Nodes (5): FocusRequester, List, TvContinueWatchingRow(), TvHomeScreen(), TvMyListRow()

### Community 79 - "Cluster 79"
Cohesion: 0.43
Nodes (3): Format, Double, String

### Community 80 - "Cluster 80"
Cohesion: 0.39
Nodes (8): AnimeDetailScreen(), AnimeHeader(), AnimeSynopsis(), EpisodeCell(), Boolean, Float, Int, String

### Community 81 - "Cluster 81"
Cohesion: 0.12
Nodes (15): 10. `navigation/AppNavHost.kt`, 1. `ui/common/SectionHeader.kt`, 2. `navigation/NavRoutes.kt`, 3. `data/remote/TMDBApi.kt`, 4. `tmdb/TMDBClient.kt`, 5. `ui/continuewatching/ContinueWatchingScreen.kt` (nuovo), 6. `ui/continuewatching/ContinueWatchingViewModel.kt` (nuovo), 7. `ui/sectionlist/SectionListScreen.kt` (nuovo) (+7 more)

### Community 82 - "Cluster 82"
Cohesion: 0.29
Nodes (9): Boolean, Color, Dp, Float, Modifier, Shape, tvFocusFrame(), tvFocusRing() (+1 more)

### Community 83 - "Cluster 83"
Cohesion: 0.29
Nodes (7): App Launcher Icon (hdpi) - Android robot logo on green grid background, ic_launcher_round.webp (hdpi, round launcher icon), ic_launcher.webp (mdpi) - green square icon with white Android robot silhouette, ic_launcher.webp (xhdpi) - Android default launcher icon: white Android robot head silhouette with antennae on a teal/green square background with faint grid pattern, ic_launcher_round.webp (xhdpi) - round Android launcher icon, Default Android Studio placeholder launcher icon (green background, robot mascot), Streamo Android App

### Community 85 - "Cluster 85"
Cohesion: 0.33
Nodes (4): AppModule, Context, ExoPlayer, DefaultTrackSelector

### Community 86 - "Cluster 86"
Cohesion: 0.50
Nodes (3): HomeSection, HomeSections, List

### Community 87 - "Cluster 87"
Cohesion: 0.60
Nodes (5): Anime, Home, RailTab, Search, Watchlist

### Community 88 - "Cluster 88"
Cohesion: 0.29
Nodes (5): Boolean, List, StateFlow, String, SectionListViewModel

### Community 89 - ".loadCurrentSource"
Cohesion: 0.38
Nodes (6): Boolean, Int, List, Modifier, Top10Card(), Top10Row()

### Community 91 - "Cluster 91"
Cohesion: 0.40
Nodes (3): ImageModule, Context, ImageLoader

### Community 93 - "Cluster 93"
Cohesion: 0.13
Nodes (14): 1. `data/preferences/SettingsDataStore.kt`, 2. `ui/settings/SettingsViewModel.kt`, 3. `ui/settings/SettingsScreen.kt`, 4. `ui/player/PlayerViewModel.kt`, 5. `ui/player/PlayerScreen.kt`, 6. (Opzionale) `data/backup/BackupManager.kt`, Considerazioni / rischi, File coinvolti (+6 more)

### Community 94 - "Cluster 94"
Cohesion: 0.60
Nodes (4): isLandscapeTablet(), isPortraitTablet(), Boolean, Int

### Community 95 - "Cluster 95"
Cohesion: 0.13
Nodes (14): 1. Hero carousel in Home — *alta priorità, alto impatto visivo*, 2. Top 10 row — *alta priorità*, 3. Filtri Watchlist — *alta priorità*, 4. Info su card + toggle — *media priorità*, 5. Cap qualità streaming — *media priorità*, 6 + 7. Schermata Impostazioni avanzate + locale provider — *media priorità*, 8. WARP badge nel player — *bassa priorità*, Allineamento layout Android ↔ iOS Streamo (+6 more)

### Community 96 - "SectionHeader"
Cohesion: 0.33
Nodes (5): ImageVector, Modifier, String, Unit, SectionHeader()

### Community 97 - "Size"
Cohesion: 0.50
Nodes (3): String, Size, TMDBImage

### Community 99 - "Cluster 99"
Cohesion: 0.45
Nodes (10): Boolean, FocusRequester, String, RowContent(), SectionHeader(), SettingsToggleRow(), SettingsValueRow(), streamingQualityLabel() (+2 more)

### Community 103 - "TvAnimeDetailScreen"
Cohesion: 0.38
Nodes (9): AnimeDetailHeader(), Boolean, Float, FocusRequester, Int, String, TvAnimeDetailScreen(), TvEpisodeCell() (+1 more)

### Community 131 - "Piano: Modali glass con blur reale (inline overlay)"
Cohesion: 0.15
Nodes (12): 1. `ui/common/GlassDialog.kt`, 2. `ui/common/GlassTopBarScaffold.kt`, 3. Schermate con `GlassTopBarScaffold`, 4. Schermate con `hazeSource` locale, 5. Dialog wrapper, File coinvolti, Obiettivo, Passaggi (+4 more)

### Community 132 - "Piano: Allineare "Salta intro" all'implementazione iOS"
Cohesion: 0.20
Nodes (9): Considerazioni, Cosa fa iOS, Design UI Android, File coinvolti, Modifiche, Nuovi file, Obiettivo Android, Passaggi (+1 more)

### Community 133 - "PlayerViewModel.kt"
Cohesion: 0.27
Nodes (7): Chromecast, Dlna, Lan, onPlayerError(), PendingCastTarget, SkipPrompt, PlaybackException

### Community 134 - "SeasonChip"
Cohesion: 0.40
Nodes (4): Boolean, Int, Modifier, SeasonChip()

### Community 135 - "Repository Guidelines"
Cohesion: 0.25
Nodes (7): Build, Test & Development Commands, Coding Style & Naming Conventions, Commit & Pull Request Guidelines, Project Structure & Module Organization, Repository Guidelines, Security & Configuration Tips, Testing Guidelines

### Community 175 - "HeroControls"
Cohesion: 0.50
Nodes (7): ConfirmDialog(), Boolean, FocusRequester, String, SectionHeader(), SettingsValueRow(), TvCacheManagementScreen()

## Ambiguous Edges - Review These
- `Streamo Android App` → `ic_launcher.webp (xhdpi) - Android default launcher icon: white Android robot head silhouette with antennae on a teal/green square background with faint grid pattern`  [AMBIGUOUS]
  app/src/main/res/mipmap-xhdpi/ic_launcher.webp · relation: is_launcher_icon_of

## Knowledge Gaps
- **207 isolated node(s):** `ProviderMapping`, `StreamSource`, `TmdbCastMember`, `TmdbCredits`, `TmdbVideo` (+202 more)
  These have ≤1 connection - possible missing edges or undocumented components.
- **66 thin communities (<3 nodes) omitted from report** — run `graphify query` to explore isolated nodes.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **What is the exact relationship between `Streamo Android App` and `ic_launcher.webp (xhdpi) - Android default launcher icon: white Android robot head silhouette with antennae on a teal/green square background with faint grid pattern`?**
  _Edge tagged AMBIGUOUS (relation: is_launcher_icon_of) - confidence is low._
- **Why does `Text` connect `TV Player Screen` to `Provider Client Resolution`, `Brand Buttons & TMDB Reviews`, `SeasonChip`, `Glass Card Composables`, `Download DAO`, `Glass Dialogs & Alerts`, `Image Placeholder & TV Media Card`, `Download Entry & Downloads VM`, `History DAO`, `Cast Banner UI`, `Anime Detail Screen`, `Glass Top Bar`, `TV Navigation Drawer`, `Cluster 43`, `Cluster 45`, `Cluster 46`, `Cluster 47`, `Cluster 48`, `HeroControls`, `Cluster 50`, `Cluster 52`, `Cluster 55`, `Cluster 57`, `Cluster 58`, `Cluster 61`, `Cluster 64`, `Cluster 68`, `Cluster 72`, `Cluster 74`, `Cluster 78`, `Cluster 80`, `.loadCurrentSource`, `SectionHeader`, `Cluster 99`, `TvAnimeDetailScreen`?**
  _High betweenness centrality (0.275) - this node is a cross-community bridge._
- **Why does `PlayerViewModel` connect `Player ViewModel & MediaSession` to `TMDB DTO Models`, `Brand Buttons & TMDB Reviews`, `PlayerViewModel.kt`, `DLNA Cast Manager`, `Cluster 70`, `LAN Cast Client (app-to-app)`, `Cluster 73`, `Cluster 51`, `Cluster 85`, `Cluster 56`, `Player ViewModel Cast Modes`, `Cast Banner & Section List VMs`?**
  _High betweenness centrality (0.190) - this node is a cross-community bridge._
- **Why does `TmdbItem` connect `TMDB DTO Models` to `.loadCurrentSource`, `Cluster 68`, `Search Screen & Glass Filters`, `Cluster 88`, `Cluster 42`, `Cluster 47`, `Player ViewModel & MediaSession`, `History DAO`, `Cast Banner UI`, `Cluster 57`, `Watchlist DAO`?**
  _High betweenness centrality (0.133) - this node is a cross-community bridge._
- **Are the 119 inferred relationships involving `Text` (e.g. with `CastBanner()` and `GlassBottomBar()`) actually correct?**
  _`Text` has 119 INFERRED edges - model-reasoned connections that need verification._
- **What connects `ProviderMapping`, `StreamSource`, `TmdbCastMember` to the rest of the system?**
  _218 weakly-connected nodes found - possible documentation gaps or missing edges._
- **Should `TMDB DTO Models` be split into smaller, more focused modules?**
  _Cohesion score 0.05603960396039604 - nodes in this community are weakly interconnected._