...
/**
 * Phone-side client per Obsidian-to-Obsidian cast.
 *
 * - NSD discovery di servizi [_streamo._tcp] sulla LAN
 * - HTTP commands (form-encoded) verso il server LanCastServer sulla TV
 */
class LanCastClient {
    ...
    private fun LanRenderer.baseUrl(): String {
    ...
    /**
     * Scopre i dispositivi Obsidian sulla LAN via NSD (_streamo._tcp).
     * Raccoglie per [timeoutMs] millisecondi, poi restituisce la lista.
     */
    suspend fun discover(context: Context, timeoutMs: Long = 5000): List<LanRenderer> =
        withContext(Dispatchers.IO) {
            ...
            val renderers = mutableListOf<LanRenderer>()
            ...
            fun newResolveListener() = object : NsdManager.ResolveListener {
                ...
                override fun onServiceResolved(serviceInfo: NsdServiceInfo?) {
                    ...
                    val port = info.port
                    if (port <= 0) return
                    val name = info.serviceName ?: "Project Obsidian"
                    val r = LanRenderer(friendlyName = name, host = host, port = port)
    ...
    suspend fun play(renderer: LanRenderer, media: CastMedia, startPositionMs: Long): Boolean =
        withContext(Dispatchers.IO) {
    ...
    suspend fun pause(renderer: LanRenderer): Boolean = withContext(Dispatchers.IO) {
        post(renderer, "/pause", FormBody.Builder().build())
    }

    suspend fun resume(renderer: LanRenderer): Boolean = withContext(Dispatchers.IO) {
        post(renderer, "/resume", FormBody.Builder().build())
    }

    suspend fun stop(renderer: LanRenderer): Boolean = withContext(Dispatchers.IO) {
        post(renderer, "/stop", FormBody.Builder().build())
    }

    suspend fun seek(renderer: LanRenderer, positionMs: Long): Boolean = withContext(Dispatchers.IO) {
        val body = FormBody.Builder()
    ...
    suspend fun status(renderer: LanRenderer): LanStatus? = withContext(Dispatchers.IO) {
        try {
            ...
            http.newCall(req).execute().use { resp ->
                ...
                gson.fromJson(json, LanStatus::class.java)
            }
        } catch (e: Exception) {
    ...
    private fun post(renderer: LanRenderer, path: String, body: FormBody): Boolean = try {
        val req = Request.Builder()
    ...
    companion object {
        private const val TAG = "LanCastClient"
