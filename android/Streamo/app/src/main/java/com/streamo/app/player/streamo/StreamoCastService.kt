...
/**
 * Foreground service che mantiene attivo StreamoCastServer sulla TV.
 *
 ...
 */
class StreamoCastService : Service() {
    ...
    private fun observeCommands() {
        serviceScope.launch {
            StreamoCastReceiver.commands.collect { cmd ->
                if (cmd is StreamoCommand.Play) launchPlayer()
            }
        }
    }

    private fun launchPlayer() {
        // TvRootView, appena visibile, legge StreamoCastReceiver.pendingPlay e apre il player.
    ...
    private fun startServer() {
        ...
        if (StreamoCastReceiver.isRunning) return

        if (!StreamoCastReceiver.start()) {
        ...
        registerNsd()
        acquireLocks()
        Log.i(TAG, "server running on port ${StreamoCastReceiver.listeningPort}")
    }
    ...
    private fun registerNsd() {
        nsdManager = (getSystemService(Context.NSD_SERVICE) as? NsdManager)?.also { nsd ->
            val serviceInfo = NsdServiceInfo().apply {
                serviceName = "Streamo - ${Build.MODEL}"
                serviceType = SERVICE_TYPE
                port = StreamoCastReceiver.listeningPort
            }
    ...
    private fun createNotificationChannel() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            val channel = NotificationChannel(
                ...
                description = "Mantiene attiva la ricezione di trasmissioni da altri dispositivi Streamo"
                setShowBadge(false)
            }
    ...
    companion object {
        private const val TAG = "StreamoCastService"
        ...
        fun startIfTv(context: Context) {
            context.startForegroundService(Intent(context, StreamoCastService::class.java))
        }
    }
}
