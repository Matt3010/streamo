...
class LanCastServer(
    port: Int,
    private val emit: (LanCommand) -> Boolean,
    private val currentStatus: MutableStateFlow<LanStatus>
) : NanoHTTPD(port) {
    ...
    override fun serve(session: IHTTPSession): Response {
        return try {
            ...
            when {
                ...
                path == "/pause" && session.method == Method.POST -> handleCommand(LanCommand.Pause, "paused")
                path == "/resume" && session.method == Method.POST -> handleCommand(LanCommand.Resume, "playing")
                path == "/stop" && session.method == Method.POST -> handleCommand(LanCommand.Stop, "stopped")
    ...
    private fun handleCommand(cmd: LanCommand, statusLabel: String): Response {
        val sent = emit(cmd)
        if (!sent) {
            return jsonResponse(Response.Status.SERVICE_UNAVAILABLE, mapOf("error" to "player busy"))
        }
    ...
    private fun handleSeek(session: IHTTPSession): Response {
        ...
        val sent = emit(LanCommand.Seek(posMs))
        if (!sent) {
            return jsonResponse(Response.Status.SERVICE_UNAVAILABLE, mapOf("error" to "player busy"))
        }
    ...
    companion object {
        private const val TAG = "LanCastServer"
    }
}
