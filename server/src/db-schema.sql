-- Postgres schema for streamingimmunity. Booleans are kept as INTEGER (0/1)
-- and timestamps as BIGINT (Unix epoch seconds) to match the SQLite shape
-- consumed by the frontend; no application-level conversions are needed.
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_history_unique ON history(user_id, tmdb_id, media_type, season, episode);
